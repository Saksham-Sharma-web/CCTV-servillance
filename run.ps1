# IBVAP Root Launcher (Cross-Device / Portable)
param(
...
Write-Host "  IBVAP Root Launcher - Multi-Device Support     " -ForegroundColor Cyan
Write-Host "=================================================" -ForegroundColor Cyan
...
$candidateVenvs = @(
    ...
    (Join-Path $PSScriptRoot "ibvap_rust\.venv")
)
...
if ($InstallReqs) {
    ...
    if ((Test-Path $reqFile) -and $detectedPython) {
        Write-Host "[INFO] Installing/updating requirements from $reqFile..." -ForegroundColor Cyan
        & "$detectedPython" -m pip install -r "$reqFile"
    }
}

# 4. Launch Rust Command Center
$rustDir = Join-Path $PSScriptRoot "ibvap_rust"
